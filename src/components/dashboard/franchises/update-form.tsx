...
import { createFranchiseAction, updateFranchiseAction } from "@/actions/franchises"
...
import { Category, Country, Franchise, Unit } from "@/types"
...
export const UpdateFranchiseForm = ({ franchise, categories, countries, units }: Props) => {
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      number_of_labors: franchise.number_of_labors,
      establish_year: franchise.establish_year,
      center_office: franchise.center_office,
      space_required: {
        unit_id: franchise.space_required.unit_id,
        value: franchise.space_required.value
      },
      equipment_cost: {
        unit_id: franchise.equipment_cost.unit_id,
        value: franchise.space_required.value
      },
      training_period: {
        unit_id: franchise.training_period.unit_id,
        value: franchise.training_period.value
      },
      contract_period: {
        unit_id: franchise.contract_period.unit_id,
        value: franchise.contract_period.value
      },
      franchise_characteristics: {
        franchise_fees: franchise.franchise_characteristic.franchise_fees,
        royalty_fees: franchise.franchise_characteristic.royalty_fees,
        marketing_fees: franchise.franchise_characteristic.marketing_fees,
        investments_cost: franchise.franchise_characteristic.investments_cost
      }
    }
  })

  const updateMutatuion = useMutation({
  ...
  const handleSubmit = () => {
    ...
    updateMutatuion.mutate({
      data: form.getValues() as any as z.infer<typeof FranchiseSchema.Update>,
      file,
      video
    })
  }
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className='space-y-4'>
          ...
          <div className='grid xl:grid-cols-5 grid-cols-1 gap-2'>
            <div className='col-span-1'>
              <SelectField
                ...
                control={form.control}
                label='الوحدة'
                defaultValue={franchise.space_required.unit_id.toString()}
              >
                {units.map((unit) => (
          ...
          <div className='grid xl:grid-cols-5 grid-cols-1 gap-2'>
            <div className='col-span-1'>
              <SelectField
                ...
                control={form.control}
                label='الوحدة'
                defaultValue={franchise.equipment_cost.unit_id.toString()}
              >
                {units.map((unit) => (
          ...
          <div className='grid xl:grid-cols-5 grid-cols-1 gap-2'>
            <div className='col-span-1'>
              <SelectField
                ...
                control={form.control}
                label='الوحدة'
                defaultValue={franchise.training_period.unit_id.toString()}
              >
                {units.map((unit) => (
          ...
          <div className='grid xl:grid-cols-5 grid-cols-1 gap-2'>
            <div className='col-span-1'>
              <SelectField
                ...
                control={form.control}
                label='الوحدة'
                defaultValue={franchise.contract_period.unit_id.toString()}
              >
                {units.map((unit) => (
        ...
        <LoadingButton loading={updateMutatuion.isPending}>تعديل</LoadingButton>
      </form>
    </Form>
  )
}
