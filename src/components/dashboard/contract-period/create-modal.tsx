...
import { createContractPeriodAction } from "@/actions/contract-periods"

export const CreateContractPeriodModal = () => {
  ...
  const createMutation = useMutation({
    mutationFn: ({ data }: { data: z.infer<typeof UnitSchema.Create> }) =>
      createContractPeriodAction(data),
    onSuccess: (data) => {
      showResponse(data, () => {
        setOpen(false)
      })
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>انشاء فترة عقد</DialogTitle>
        </DialogHeader>
